...
bool Window::open(int x, int y, int newflags, int newrenflags) {
  ...
  if(!win) {
    ...
    win = SDL_CreateWindow(title.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, sx, sy, flags);
    if(!win) return false;
...
int lh_openwindow(lua_State *L) {
  checkArg(L, 5, "openwindow");

  return noteye_retBool(L, 
    luaO(1, Window)->open(luaInt(2), luaInt(3), luaInt(4), luaInt(5))
    );  
  }
